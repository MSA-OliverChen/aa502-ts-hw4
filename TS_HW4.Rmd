...
esm_fit <- train.ts |>
  model(
    #Some with just weekly seasonality
    ...
    #Some with both week/year (specifying period="year" gave error)
    'HWAddYear' = ETS(daily_total ~ error("A") + trend("A") + season("A")),
    'HWMultYear' = ETS(daily_total ~ error("M") + trend("A") + season("M")),
    'NoTrendYear' = ETS(daily_total ~ error("A") + trend("N") + season("A")),
  )
```
...
```
```{r}
#Idea: roll it up by month so we can see yearly trend, then interpolate to get days in between
```
